...
class CollisionGridModel(nn.Module):
    ...
    def __init__(self, args, infer=False):
        ...
        self.tensor_embedding_layer = nn.Linear(self.num_TTC*self.num_sector, self.embedding_size) 
        ...
        # Linear layer to embed the social tensor of vehicles around the target pedestrian
    ...
    def getSocialTensor2(self, grid, hidden_states, grid_TTC):
        ...
        params:
        grid : Grid masks
        hidden_states : Hidden states of all peds / OR the embedded state of vehicles /OR the direct action of the neighboring agents
        '''
        # Number of peds
        numNodes = grid.size()[0]
        ...
        for node in range(numNodes):
            ...
            # With the restriction of having only one agent in each occupancy grid cell, we keep the more critical one,
            # defined as having the lowest TTC
            grid_updated = self.OnePerGridCell(grid[node], grid_TTC[node])
            # if you do not want to count for just the most critical one and instead some embeddings of all interacting agents then use the following line instead
            # grid_updated = grid[node]
    ...
    def OnePerGridCell(self, grid_current, grid_TTC_current):
        ...
        for col in valid_col_index:
            col_ind = col.item()
            # getting the row indexes of non-zero rows (indexs of neigboring agents residing in this specific grid cell) 
            ...
            ttc = grid_TTC_current[rows,col_ind] # a smaller tensor or only ttc of conflicting agents present in that cell
    ...
    def forward(self, *args):
        ...
        for framenum,frame in enumerate(input_data):
            ...
            ## for oblation cases comment the above line and uncomment the associated lines below
            # concat_embedded = torch.cat((input_embedded, tensor_embedded), 1)
            # concat_embedded = torch.cat((input_embedded, tensor_embedded_veh), 1)
